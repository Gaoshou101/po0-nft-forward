...
| 坑 | 后果 |
|---|---|
| **DNAT 匹配条件写入口机的公网 IP** —— 腾讯云公网 IP 是 NAT 映射的，实例网卡上只有内网 IP，入站包到 netfilter 时目的地址**已经是内网 IP** | 规则语法完全正确、`nft list ruleset` 也看得到，但**永远不命中**，客户端死活连不上 —— 最难自查的一个坑 |
...
本脚本逐条解决。**DNAT 规则只按端口匹配、不匹配公网 IP**，就是为了避开上表第一条。

### 另外两条实测结论

- **入口机的公网 TCP 出口是封死的**：实测只有 DNS（UDP 53）与腾讯内网镜像（`mirrors.tencentyun.com`）可达，`baidu.com`、`api.github.com`、`raw.githubusercontent.com`、乃至 Po0 自己的控制节点 `:443` 全部超时。所以本 README 里的 `curl` 下载步骤**在入口机上跑不通时不是慢、是没有出网**——GitHub 加速镜像也救不了。正确姿势：在本地有网的机器上下载好，再 `ssh root@入口机 'cat > 文件' < 本地文件` 推过去（多个文件先 `tar czf` 一次推更省事），或走 apt / 腾讯内网镜像。
- **Po0 的防火墙白名单挡的是所有入站端口，不只是 SSH**。开启后只有白名单里的来源 /24 能访问任何入站端口，所以**每个要用这条链路上网的客户端设备/网络都得加白**（上限 5 个 /24、FIFO 淘汰）。最省事：家宽侧软路由/常驻设备跑 `po0fw` 自动加白，并把家宽钉到 `@0` 号槽位。
