...
| 坑 | 后果 |
|---|---|
| SNAT 源地址默认取默认路由的 `src`，在 Po0 机器上就是**公网 IP** | 出口机看到的源不在 CCN 内网，回包不走内网链路 → 链路退化甚至不通 |
...
先在落地机上搭好协议并确认直连可用，再在 RFC 中转机上做一段转发（客户端临时连 RFC 中转机验证），最后在 Po0 上把目标指向 RFC 中转机的内网 IP。
...
- 不要装哆啦等转发面板
- 流量按**单向**统计
- 目标 IP 用出口机的 **CCN 内网 IP**，SNAT 用本机**内网 IP**，两者必须在同一内网
